...
public class Ioc {
    ...
    @SuppressWarnings("unchecked")
    private static <T extends EntityObject> boolean lastListImpl(Collection<T> result, EntityObject entity, Predicate<T> filter,
                                                                       Sign[] args, int index) {
        if (index >= args.length) {
            index = 0;
        }
    ...
    @SuppressWarnings("unchecked")
    private static <T extends EntityObject> void recursionImpl(EntityObject entity, Predicate<T> fun,
                                                                     Sign[] args, int index) {
        if (index >= args.length) {
